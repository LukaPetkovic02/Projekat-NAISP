...
func main() {
	var sl = &bTree.Stablo{}
	sl.InitSP(int(config.Values.Btree.MaxNode))
	record := types.CreateRecord("2131231", []byte("asfiashfgiohaso"), false)
	sl.Add(record)

	record = types.CreateRecord("4141", []byte("asfiashfgiohaso"), false)
	sl.Add(record)

	record = types.CreateRecord("555125", []byte("asfiashfgiohaso"), false)
	sl.Add(record)

	fmt.Println(sl.Get("2131231"))
	fmt.Println(sl.Get("1111"))
