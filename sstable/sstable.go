...
// func ReadFromMultipleFilesWithPrefix(prefix string) map[string]types.Record {
// 	var items, err = os.ReadDir(engine.GetBloomDir())
// 	var maps map[string]types.Record
// 	if err != nil {
// 		panic(err)
// 	}
...
// 		sumfile, errr := os.Open(engine.GetSummaryPath(item.Name()))
// 		if errr != nil {
// 			panic(err)
// 		}

// 		var summary = ReadSummaryHeader(sumfile)
// 		if prefix < string(summary.StartKey) && prefix > string(summary.EndKey) {
// 			continue
// 		}

// 		for true {
// 			var closestRecord = getClosestRecordWithPrefix(prefix, summary.EndKey, sumfile)
...
// 				panic(err)
// 			}
// 			var index = readIndex(file, closestRecord.Offset, prefix)
// 			file, err = os.Open(engine.GetSSTablePath(item.Name()))
// 			if err != nil {
// 				panic(err)
// 			}
// 			file.Seek(int64(index.Offset), 0)
// 			var record = types.ReadRecord(file)
// 			if _, exists := maps[record.Key]; exists {
// 				if maps[record.Key].Timestamp < record.Timestamp {
// 					maps[record.Key] = record
// 				}
// 			} else {
// 				maps[record.Key] = record
// 			}
...
// func ReadFromSingleFileWithPrefix(key string) map[string]types.Record {
// 	var items, err = os.ReadDir(engine.GetTableDir())
// 	if err != nil {
// 		panic(err)
// 	}
